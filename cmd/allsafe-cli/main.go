...
func interactiveLogin() {
    ...
    // Try to log in initially with just username and password
    ...
    if err != nil {
        if strings.Contains(err.Error(), "MFA required") {
            ...
            // Retry login with the TOTP code
...
// It now accepts an optional totpCode for MFA.
func login(username, password, totpCode string) error {
    ...
    // Create the login data map.
    loginData := map[string]string{
        "username": username,
        "password": password,
    }

    // If a TOTP code is provided, add it to the login data.
    if totpCode != "" {
    ...
    if resp.StatusCode != http.StatusOK {
        ...
        // Check for the specific MFA required error message
    ...
    if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
        // If there is no JSON body, we assume the login was successful.
        currentUser = &User{Username: username, AuthTime: time.Now()}
        fmt.Printf("[allsafe-cli] Login successful. *Welcome, %s!*\n", username)
        return nil
    }

    // A token-based response indicates a successful login.
    currentUser = &User{Username: username, AuthTime: time.Now()}
