...
func main() {
	...
	rootCmd.PersistentFlags().StringVarP(&proxyAddress, "proxy", "p", "", "Proxy server address (overrides config)")
...
func initConfig() {
	// Use Viper to find and load a dedicated configuration file for the CLI.
	viper.SetEnvKeyReplacer(strings.NewReplacer(".", "_", "-", "_"))
	viper.AutomaticEnv()

	// Only search for the dedicated CLI config file if the user hasn't provided a flag.
	if proxyAddress == "" {
		viper.AddConfigPath(".")
		viper.AddConfigPath(filepath.Join(".."))
		viper.AddConfigPath(filepath.Join("/etc", "allsafe-cli"))
		viper.SetConfigName("allsafe-cli")
		viper.SetConfigType("yaml")

		if err := viper.ReadInConfig(); err == nil {
			log.Printf("Using config file: %s", viper.ConfigFileUsed())
			proxyAddress = viper.GetString("proxy_url")
		} else {
			if _, ok := err.(viper.ConfigFileNotFoundError); ok {
				log.Println("Warning: No 'allsafe-cli.yaml' config file found. You must provide the proxy address with the --proxy flag.")
			} else {
	...
	// Final check: if after checking both the flag and the config, the address is still empty, exit.
	if proxyAddress == "" {
		log.Fatal("Fatal: Proxy address is not set. Please provide it using the --proxy flag or in an 'allsafe-cli.yaml' file.")
	}

	log.Printf("Proxy Address for CLI is set to: %s", proxyAddress)
}
...
func interactiveLogin() {
	...
	if err != nil {
		if strings.Contains(err.Error(), "MFA required") {
			fmt.Printf("Totp Authentication Code: ")
			totpCode, _ := reader.ReadString('\n')
			totpCode = strings.TrimSpace(totpCode)
			fmt.Println()

			if totpCode == "" {
				fmt.Println("TOTP code cannot be empty.")
				return
			}

			err = login(username, password, totpCode)
			if err != nil {
				fmt.Println(err)
			}
		} else {
...
// login authenticates the user with the Allsafe Access proxy.
func login(username, password, totpCode string) error {
	...
	if err != nil {
		return fmt.Errorf("error: Failed to connect to proxy: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		if strings.Contains(body, "MFA required") {
			return fmt.Errorf("login failed: MFA required")
		}
		return fmt.Errorf("login failed (Status: %d): %s", resp.StatusCode, body)
	}
